...
for json_file in json_files:
    ...
    for item in data:
        ...
        cropped_pdf_path = os.path.join(output_folder, f'cropped_{name}.pdf')
        with fitz.open() as new_pdf:
            new_pdf.insert_pdf(pdf_document, from_page=page_num, to_page=page_num)
            new_pdf.save(cropped_pdf_path)

        # Save the caption to a text file with UTF-8 encoding
        caption_txt_path = os.path.join(output_folder, f'caption_{name}.txt')
